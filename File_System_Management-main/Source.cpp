...
Directory_Entry::Directory_Entry(string name, char attr, int F_cluster) {
    bool ok = name.find(".txt") != string::npos && name.find(".txt") == name.size() - 4;
    if (ok) {
        int i = 0;
    ...
    if (this->First_cluster != 0) {
        ...
        for (int i = 0; i < Directory_Tabledata.size(); i++) {
            ...
            if (Directory_Entrydata.size() == 32) {
                if (isalpha(Directory_Entrydata[0]))
...
void File_Entry::writeFile() {
    ...
    vector<char>Contentdata(Content.begin(), Content.end());
    int numOfrequiredclusters = (Contentdata.size() + 1023) / 1024;
    if (numOfrequiredclusters <= VDisk.Fat.getAvailable_Clusters()) {
        if (First_cluster != 0)
    ...
    vector<vector<char>>blocks(numOfrequiredclusters);
    for (int i = 0; i < numOfrequiredclusters; i++) {
        for (int j = i * 1024; j < min((int)Contentdata.size(), (i + 1) * 1024); j++)
            blocks[i].push_back(Contentdata[j]);
    }
...
public:
    ...
    void ShowDir() {
        //cout << "Dir table size" << CurrentDir->Directory_Table.size() << "\n";
        ...
        for (auto it : CurrentDir->Directory_Table) {
            ...
            {
                string siz = to_string(it.File_size * 1024);
                s += siz;
                s += string(8 - siz.size(), ' ');
                sumSize += it.File_size;
                ctrFile++;
            }
            else {
                s += "<DIR>   ";
        ...
        cout << string(8, ' ') << ctrFile << " File(s)   " << sumSize * 1024 << " bytes\n";
